...
import { getCarbonProject } from "./graphql/queries";
import { updateCarbonProject } from "./graphql/mutations";
...
export default function CarbonProjectUpdateForm(props) {
  const {
    id: idProp,
    carbonProject: carbonProjectModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = carbonProjectRecord
      ? { ...initialValues, ...carbonProjectRecord }
      : initialValues;
    setProjectName(cleanValues.projectName);
    setProjectType(cleanValues.projectType);
    setEstAnnualMitigations(cleanValues.estAnnualMitigations);
    setProjectStatus(cleanValues.projectStatus);
    setScale(cleanValues.scale);
    setCountry(cleanValues.country);
    setBusinessOwner(cleanValues.businessOwner);
    setProjectOwners(cleanValues.projectOwners ?? []);
    setCurrentProjectOwnersValue("");
    setCarbonDevelopers(cleanValues.carbonDevelopers ?? []);
    setCurrentCarbonDevelopersValue("");
    setSupportingPartners(cleanValues.supportingPartners ?? []);
    setCurrentSupportingPartnersValue("");
    setAuditors(cleanValues.auditors ?? []);
    setCurrentAuditorsValue("");
    setSdgs(cleanValues.sdgs ?? []);
    setCurrentSdgsValue("");
    setCreditIssuanceDate(cleanValues.creditIssuanceDate);
    setTotalCreditsIssued(cleanValues.totalCreditsIssued);
    setSector(cleanValues.sector);
    setMethodology(cleanValues.methodology);
    setSustainableCert(cleanValues.sustainableCert);
    setValidationBody(cleanValues.validationBody);
    setVerficationBody(cleanValues.verficationBody);
    setProjectDuration(cleanValues.projectDuration);
    setRegistry(cleanValues.registry);
    setValidationCriteria(cleanValues.validationCriteria ?? []);
    setCurrentValidationCriteriaValue("");
    setVerificationCriteria(cleanValues.verificationCriteria ?? []);
    setCurrentVerificationCriteriaValue("");
    setCity(cleanValues.city);
    setAddress(cleanValues.address);
    setGeographicalRegion(cleanValues.geographicalRegion);
    setCoordinates(cleanValues.coordinates);
    setGoogleMapLink(cleanValues.googleMapLink);
    setCreditsRetired(cleanValues.creditsRetired);
    setCurrentCreditHolders(cleanValues.currentCreditHolders ?? []);
    setCurrentCurrentCreditHoldersValue("");
    setProjectInsights(cleanValues.projectInsights);
    setProjectDescFileLink(cleanValues.projectDescFileLink);
    setReviewReportFileLink(cleanValues.reviewReportFileLink);
    setValidationPlanFileLink(cleanValues.validationPlanFileLink);
    setValidationReportFileLink(cleanValues.validationReportFileLink);
    setMediaFiles(cleanValues.mediaFiles ?? []);
    setCurrentMediaFilesValue("");
    setErrors({});
  };
  const [carbonProjectRecord, setCarbonProjectRecord] = React.useState(
    carbonProjectModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getCarbonProject.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getCarbonProject
        : carbonProjectModelProp;
      setCarbonProjectRecord(record);
    };
    queryData();
  }, [idProp, carbonProjectModelProp]);
  React.useEffect(resetStateValues, [carbonProjectRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          estAnnualMitigations: estAnnualMitigations ?? null,
          projectStatus,
          scale: scale ?? null,
          country: country ?? null,
          businessOwner: businessOwner ?? null,
          projectOwners: projectOwners ?? null,
          carbonDevelopers: carbonDevelopers ?? null,
          supportingPartners: supportingPartners ?? null,
          auditors: auditors ?? null,
          sdgs: sdgs ?? null,
          creditIssuanceDate: creditIssuanceDate ?? null,
          totalCreditsIssued: totalCreditsIssued ?? null,
          sector: sector ?? null,
          methodology: methodology ?? null,
          sustainableCert: sustainableCert ?? null,
          validationBody: validationBody ?? null,
          verficationBody: verficationBody ?? null,
          ...
          currentCreditHolders: currentCreditHolders ?? null,
          projectInsights: projectInsights ?? null,
          projectDescFileLink: projectDescFileLink ?? null,
          reviewReportFileLink: reviewReportFileLink ?? null,
          validationPlanFileLink: validationPlanFileLink ?? null,
          validationReportFileLink: validationReportFileLink ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateCarbonProject.replaceAll("__typename", ""),
            variables: {
              input: {
                id: carbonProjectRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "CarbonProjectUpdateForm")}
      {...rest}
    >
      <TextField
        label="Project name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || carbonProjectModelProp)}
